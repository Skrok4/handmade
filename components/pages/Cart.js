class Cart {
  showCart() {
    ...
    if (this.getCookie("cart") && this.getCookie("cart") !== "[]") {
      ...
      this.element.innerHTML = "<h2>Кошик пустий</h2>";
    }
  ...
  render() {
    ...
    data.forEach((el) => {
      if (cartUnicElements.indexOf(el.id) !== -1) {
        ...
        item.innerHTML = `
               ...
               <td class="order-list__item--subtotal">${(
                 el.price * count
               ).toFixed(2)} ₴</td>
            `;
        this.element
    ...
    rowTotalSum.innerHTML = `
         <td colspan="5" style="text-align: right">Усього :</td>
         <td id="totalSum">${
           document.querySelector(".cart__sum").textContent
         }</td>
      `;
